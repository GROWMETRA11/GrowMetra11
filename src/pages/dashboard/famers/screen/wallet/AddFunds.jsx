      ...
      { step === 1 && ( <div>
        <div className="w-full mt-[70px]">
          {fundsType.map((item, index) => (
            <div onClick={() => {setPaymentMethod(item.title)
              setPaymentError()
            }}
              key={index}
              className={`${paymentMethod === item.title ? "bg-green-100" : "bg-white"}
               px-8 py-[56px] gap-10 flex items-center rounded-lg ${index === 2 && paymentError ? "mb-2" : "mb-10"}`}
            >
              <p>{item.icon}</p>
              <div>
          ...
          {paymentError && <div className='text-red-500 text-xl'>{paymentError}</div>}
        </div>
        ...
        { step === 2 && ( <div>
          ...
          <div className="grid grid-cols-2 gap-[31px]">
            {quickAmount.map((value,index) => (
              <div key={index} onClick={() => {setAmount(value) 
                setError("")
          ...
          <div className='flex flex-col lg:flex-row justify-start gap-10 items-center mt-[70px]'>
              <button  onClick={() => goNext()} className='bg-green-500  text-white font-medium text-[20px] rounded-md h-[50px] w-full max-w-[700px] flex justify-center items-center'>Continue</button>
        ...
        { step === 3 && ( <div>
          <div className="px-8 py-[27px] bg-white rounded-lg flex justify-between mt-[70px] ">
            ...
            <p className="text-[20px] font-normal text-gray-900">{paymentMethod}</p>
          </div>
        ...
        { step === 4 && ( <div>
          <div className='flex flex-col justify-center items-center'>
            ...
            <p className=' mb-[50px]'>₦{amount} has been added to your wallet</p>
          </div>
